...
import scala.com.atguigu.spark.core.request.bean.HotCategory
import scala.com.atguigu.spark.core.request.dao.{HotCategoryAnalysisTop10Dao, WordCountDao}
import scala.com.atguigu.spark.core.request.helper.HotCategoryAccumulator
...
class HotCategoryAnalysisTop10Service extends TService{
    ...
    /**
     * 数据分析
     * TODO 方案二 ：加缓存cache；结构优化，只使用一次reduceByKey
     * @return
     */
    def analysis2() = {
        ...
        val categoryIdToPayCountRDD = payToOneRDD.reduceByKey(_+_)
    ...
    def analysis1() = {
        ...
        val mapRDD = joinRDD1.mapValues {
            case ((clickCount, orderCont), payCount) => {
                (clickCount, orderCont, payCount)
            }
        }
